...
private static $_icuShortFormats = [
    ...
    public function init()
    {
        ...
        if ($this->message === null) {
            $this->message = 'Формат {attribute} не правильный.';
        }
        if ($this->format === null) {
            if ($this->type === self::TYPE_DATE) {
                ...
                throw new \InvalidArgumentException('Unknown validation type set for DateValidator::$type: ' . $this->type);
            }
        }
        if ($this->locale === null) {
            $this->locale = 'ru-RU';
        ...
        if ($this->timeZone === null) {
            $dt = new DateTime();
            $this->timeZone = $dt->getTimezone()->getName();
        }
        if ($this->min !== null && $this->tooSmall === null) {
            $this->tooSmall = '{attribute} должен быть не меньше чем{min}.';
        }
        if ($this->max !== null && $this->tooBig === null) {
            $this->tooBig = '{attribute} должен быть меньше чем {max}.';
        }
        ...
        if ($this->max !== null && is_string($this->max)) {
            ...
            if ($timestamp === false) {
                throw new \InvalidArgumentException("Invalid max date value: {$this->max}");
            }
            $this->max = $timestamp;
        }
        if ($this->min !== null && is_string($this->min)) {
            ...
            if ($timestamp === false) {
                throw new \InvalidArgumentException("Invalid min date value: {$this->min}");
            }
            $this->min = $timestamp;
        }
    }
    ...
    private function parseDateValueFormat($value, $format)
    {
        ...
        if (strncmp($format, 'php:', 4) === 0) {
            ...
            $format = static::convertDateIcuToPhp($format, 'date');
        }
