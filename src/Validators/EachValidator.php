 ...
 * @since 2.0.4
 */
class EachValidator extends BaseValidator
{
    /**
     * @var array|BaseValidator definition of the validation rule, which should be used on array values.
    ...
    public function init()
    {
        ...
        if ($this->message === null) {
            $this->message =  '{attribute} не верен.';
        }
    }
    ...
    private function createEmbeddedValidator($model)
    {
        ...
        if ($rule instanceof BaseValidator) {
            ...
            if (!is_object($model)) {
                $model = new Form(); // mock up context model
            }

            return BaseValidator::createValidator($rule[0], $model, $this->attributes, array_slice($rule, 1));
        }

        throw new \InvalidArgumentException('Invalid validation rule: a rule must be an array specifying validator type.');
    }
