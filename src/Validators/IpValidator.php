...
class IpValidator extends BaseValidator
{
    ...
    public function init()
    {
        ...
        if ($this->message === null) {
            $this->message = '{attribute} должен быть IP адресом.';
        }
        if ($this->ipv6NotAllowed === null) {
            $this->ipv6NotAllowed =  '{attribute} не принимает IPv6 адрес.';
        }
        if ($this->ipv4NotAllowed === null) {
            $this->ipv4NotAllowed =  '{attribute} не принимает IPv4 адрес.';
        }
        if ($this->wrongCidr === null) {
            $this->wrongCidr = '{attribute} содержит неправильный CIDR.';
        }
        if ($this->noSubnet === null) {
            $this->noSubnet =  '{attribute} не указана маска.';
        }
        if ($this->hasSubnet === null) {
            $this->hasSubnet =  '{attribute} не должно являться подсетью.';
        }
        if ($this->notInRange === null) {
            $this->notInRange = '{attribute} в недопустимом диапазоне.';
        }
    }
    ...
    private function validateSubnet($ip)
    {
        ...
        if ($this->getIpVersion($ip) === static::IPV6) {
            if ($cidr !== null) {
    ...
    {
        return static::expandIPv6Helper($ip);
    }
    ...
    {
        return static::getIpVersionHelper($ip);
    }
    ...
    {
        return static::inRangeHelper($ip . '/' . $cidr, $range);
    }
     ...
     */
    public static function inRangeHelper($subnet, $range)
    {
        list($ip, $mask) = array_pad(explode('/', $subnet), 2, null);
        list($net, $netMask) = array_pad(explode('/', $range), 2, null);

        $ipVersion = static::getIpVersionHelper($ip);
        $netVersion = static::getIpVersionHelper($net);
    ...
    public static function getIpVersionHelper($ip)
    {
    ...
    public static function expandIPv6Helper($ip)
    {
