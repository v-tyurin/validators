...
class CompareValidator extends BaseValidator
{
    ...
    public function init()
    {
        ...
        if ($this->message === null) {
            switch ($this->operator) {
                case '==':
                case '===':
                    $this->message = '{attribute} должен быть равен "{compareValueOrAttribute}".';
                    break;
                case '!=':
                case '!==':
                    $this->message ='{attribute} должен быть не равен "{compareValueOrAttribute}".';
                    break;
                case '>':
                    $this->message = '{attribute} должен быть больше "{compareValueOrAttribute}".';
                    break;
                case '>=':
                    $this->message = '{attribute} должен быть больше или равен "{compareValueOrAttribute}".';
                    break;
                case '<':
                    $this->message =  '{attribute} должен быть меньше "{compareValueOrAttribute}".';
                    break;
                case '<=':
                    $this->message = '{attribute} должен быть меньше или равен "{compareValueOrAttribute}".';
                    break;
                default:
                    throw new \InvalidArgumentException("Неизвестный оператор: {$this->operator}");
            }
        }
    }
    ...
    public function validateAttribute($model, $attribute)
    {
        ...
        if (is_array($value)) {
            $this->addError($model, $attribute,  '{attribute} невалидный.');
    ...
    {
        if ($this->compareValue === null) {
            throw new \InvalidArgumentException('CompareValidator::compareValue must be set.');
        }
